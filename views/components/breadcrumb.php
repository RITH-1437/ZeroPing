    ...
    <ol class="flex flex-wrap items-center gap-2 text-sm text-zp-muted">
        <?php foreach ($items as $idx => $item):
            ...
            <?php if (!empty($item['href']) && !$isLast): ?>
                <a href="<?= htmlspecialchars($item['href'], ENT_QUOTES, 'UTF-8') ?>" class="rounded px-1 hover:text-cyan-400 focus-ring transition-colors"><?= htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') ?></a>
            <?php else: ?>
                <span class="text-zp-white font-medium"><?= htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') ?></span>
            <?php endif; ?>
        <?php endforeach; ?>
    </ol>
</nav>
