...
<section class="rounded-2xl border border-slate-700/50 overflow-hidden bg-slate-950 shadow-lg shadow-black/10 group">
    <div class="flex items-center justify-between gap-2 px-4 py-2.5 border-b border-slate-800 bg-zp-surface">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <?php if ($language): ?>
                <span class="shrink-0 rounded-md bg-slate-800 px-2 py-0.5 text-[10px] font-medium text-zp-muted uppercase tracking-wider"><?= htmlspecialchars($language, ENT_QUOTES, 'UTF-8') ?></span>
            <?php endif; ?>
            <p class="text-xs text-zp-muted truncate"><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <button type="button" class="copy-code-btn shrink-0 text-xs px-2.5 py-1 rounded-md border border-slate-700/50 bg-white/5 text-zp-muted hover:text-zp-white hover:bg-white/10 focus-ring transition-all duration-200" data-copy-target="<?= htmlspecialchars($codeId, ENT_QUOTES, 'UTF-8') ?>">
