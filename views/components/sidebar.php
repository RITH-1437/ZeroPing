...
<aside class="hidden lg:block lg:sticky lg:top-28 h-fit rounded-2xl border border-zp-border bg-zp-surface p-4 shadow-sm max-h-[calc(100vh-8rem)] overflow-y-auto">
    ...
    <div class="relative">
        ...
        <input id="doc-search" type="search" placeholder="Search docs..." class="w-full rounded-xl border border-zp-border bg-zp-bg pl-9 pr-3 py-2 text-sm text-zp-white placeholder:text-zp-muted focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all duration-200">
    </div>
            ...
            <?php foreach ($documents as $doc): ?>
                <li data-doc-link>
                    <a
                        ...
                        class="block rounded-lg px-3 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 transition-all duration-200 <?= $currentSlug === $doc['slug'] ? 'bg-gradient-to-r from-cyan-500 to-emerald-500 text-zp-white shadow-md' : 'text-zp-muted hover:text-zp-white hover:bg-zp-surface' ?>"
                    >
