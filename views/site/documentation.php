...
<section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="mt-6 grid lg:grid-cols-[260px_minmax(0,1fr)_200px] gap-6">
        ...
        <article class="min-w-0 rounded-2xl border border-zp-border bg-zp-surface p-6 lg:p-10 shadow-sm" data-animate>
            <div class="prose prose-slate dark:prose-invert max-w-none
                prose-headings:scroll-mt-28 prose-headings:font-semibold prose-headings:text-zp-white
                prose-a:text-cyan-400 prose-a:font-medium
                prose-strong:text-zp-white
                ...
                <ul class="space-y-1 text-sm">
                    <?php foreach (($toc ?? []) as $item): ?>
                        <li style="padding-left: <?= (($item['level'] ?? 2) - 2) * 12 ?>px">
                            <a href="#<?= htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8') ?>" data-toc-link class="block rounded px-2 py-1 text-zp-muted hover:text-cyan-400 focus-ring transition-all duration-200 border-l-2 border-transparent hover:border-cyan-500/50">
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                tocLinks.forEach(l => {
                    l.classList.remove('text-cyan-400', 'font-medium', 'border-cyan-500/70');
                    l.classList.add('border-transparent');
                });
                const match = headings.find(h => h.el === entry.target);
                if (match) {
                    match.link.classList.add('text-cyan-400', 'font-medium', 'border-cyan-500/70');
