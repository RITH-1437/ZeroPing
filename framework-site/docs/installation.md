...
ZeroPing can be installed in two ways: the **Zero CLI** (recommended for the
fastest experience) or **Composer** (if it's already on your system). Both
methods generate the exact same project structure.
...
Both methods generate the exact same project structure. Use **Zero CLI** for
the fastest experience, or **Composer** if it's already installed on your
system.
...
`composer create-project rith-1437/zeroping my-app` (or copy `config/routes.php` from a fresh install).
